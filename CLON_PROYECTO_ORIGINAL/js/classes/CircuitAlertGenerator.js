...
class CircuitAlertGenerator {
    ...
    async _obtenerDatosTodosSectores() {
        ...
        for (const [nombre, sector] of Object.entries(this.sectores)) {
            try {
                const datos = await this.apiClient.obtenerDatosClima(sector.lat, sector.lon);
                if (datos) {
    ...
    _extraerDatosDia(dailyData, diaIndex) {
        ...
        return {
            ...
            precipitacion: dailyData.precipitation_sum?.[diaIndex] || 0, // Default 0 si no hay
    ...
    _generarAlertasSectorDia(nombreSector, sector, datosDia, diaIndex) {
        ...
        if (datosDia.tempMin && datosDia.tempMin < -5) {
            alertas.push({
                sector: nombreSector,
                circuito: sector.circuito,
                icono: sector.icon,
                tipo: 'frio_extremo',
                nivel: 'danger',
                titulo: '❄️ Frío Extremo',
                mensaje: `Temperatura mínima: ${Math.round(datosDia.tempMin)}°C. Riesgo alto de hipotermia.`,
                recomendaciones: 'Equipo de abrigo esencial. Evitar exposiciones prolongadas.',
                tempMax: Math.round(datosDia.tempMax),
                tempMin: Math.round(datosDia.tempMin),
                vientoMax: Math.round(datosDia.vientoMax || 0),
                precipitacion: datosDia.precipitacion || 0
            });
        } else if (datosDia.tempMin && datosDia.tempMin < 0) {
            alertas.push({
                sector: nombreSector,
                circuito: sector.circuito,
                icono: sector.icon,
                ...
                mensaje: `Temperatura mínima: ${Math.round(datosDia.tempMin)}°C. Protección térmica necesaria.`,
